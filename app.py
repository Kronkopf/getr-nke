...
IMMER_VORHANDEN = [
    "Orangen", "Zitronen", "Wasser", "Milch", "Eiswürfel"
]
...
def hole_vorschlaege(daten: list[dict]) -> str:
    ...
    prompt = f"""Du bist ein freundlicher Barkeeper. Basierend auf dem aktuellen Getränkebestand und den immer vorhandenen Zutaten, schlage 5-8 mögliche Getränke für Gäste vor.
...
Gib die Vorschläge als übersichtliche Liste aus. Für jedes Getränk:
- Name des Getränks mit passendem Emoji
- Kurze Beschreibung (1 Satz)
- Benötigte Zutaten aus dem Bestand

Halte es freundlich und einladend, als ob du Gästen die Karte vorliest. Antworte auf Deutsch."""
    ...
    try:
        ...
        return f"❌ Fehler beim Laden der Vorschläge: {e}"
...
# ── 🍹 KI-Bereich: Getränkekarte für Gäste ────────────────────────────────────
with st.expander("🍹 **Getränkekarte für Gäste** — Was können wir anbieten?", expanded=False):
    st.caption(f"Immer im Haus: {', '.join(IMMER_VORHANDEN)}")

    if st.button("✨ Getränkevorschläge generieren", use_container_width=True, type="primary"):
        with st.spinner("🤵 Der Barkeeper schaut in den Keller..."):
            vorschlaege = hole_vorschlaege(daten)
            st.session_state["vorschlaege"] = vorschlaege

    if "vorschlaege" in st.session_state:
        st.markdown("---")
        st.markdown(st.session_state["vorschlaege"])
        st.caption("_KI-Vorschläge basierend auf aktuellem Bestand · powered by Llama 4_")
