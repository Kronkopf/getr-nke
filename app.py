...
def hole_vorschlaege(daten: list[dict]) -> str:
    ...
    prompt = f"""Du bist ein freundlicher Barkeeper. Basierend auf dem aktuellen Getränkebestand und den immer vorhandenen Zutaten, schlage genau 10 mögliche Getränke für Gäste vor.
...
Die Vorschläge sollen von einfach bis mittel-komplex reichen (keine aufwändigen Cocktails). Für jedes Getränk:
- Name mit passendem Emoji
- Eine kurze einladende Beschreibung (1 Satz)
- Zutaten

Antworte auf Deutsch, freundlich und einladend wie eine Getränkekarte."""
